    ...
    axios
      ...
      .then(response => {
        if (response.data.success) {
          ...
          showToast(
            'success',
            'You have been successfully hired by this pet owner',
          );
        } else {
          showToast('error', response.data.message);
        }
                ...
                <View
                  ...
                  <View style={{flexDirection: 'row', width: '100%'}}>
                    {item?.pet_images?.length > 0 ? (
                      <Image
                        source={{uri: `${BasUrl}/${item?.pet_images[0]}`}}
                        style={{
                          width: 60,
                          height: 60,
                          borderRadius: 4,
                    ...
                    <View style={{marginLeft: 10}}>
                      <Text
                        style={[
                          styles.HeadingText,
                          {fontWeight: 'bold'},
                        ]}>{`${item?.pet_nickname}, ${item?.pet_age}`}</Text>
                    ...
                    <TouchableOpacity
                      onPress={() =>
                        navigation.navigate('ChatScreen', {
                          otherUserId: item.pet_owner_sender_id,
                          requestAccepted: false,
                          userData: {user_name: item.pet_owner_name},
                        })
                      }
                      style={{
                        position: 'absolute',
